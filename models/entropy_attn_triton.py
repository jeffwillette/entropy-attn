...
def _attn_fwd_inner(acc, l_i, m_i, e_i, q,  #
    ...
    for start_n in tl.range(lo, hi, BLOCK_N, warp_specialize=warp_specialize):
        ...
        if STAGE == 2:
            ...
            qk = qk * qk_scale[:, None] + tl.where(mask, 0, -1.0e6)
            m_ij = tl.maximum(m_i, tl.max(qk, 1))
            qk -= m_ij[:, None]
        else:
            m_ij = tl.maximum(m_i, tl.max(qk, 1) * qk_scale)
            qk = qk * qk_scale[:, None] - m_ij[:, None]
        p = tl.math.exp2(qk)
...
def _host_descriptor_pre_hook(nargs):
    ...
    if nargs["FP8_OUTPUT"]: # FP8_OUTPUT
...
def _attn_fwd(sm_scale, M, E,  #
    ...
    me_off = off_z * H * N_CTX + off_h * N_CTX + offs_m
    # m_ptrs = M + off_hz * N_CTX + offs_m
    # e_ptrs = E + off_hz * N_CTX + offs_m
    tl.store(M + me_off, m_i, mask=offs_m < N_CTX)
    tl.store(E + me_off, e_i, mask=offs_m < N_CTX)
...
class _attention(torch.autograd.Function):
    ...
    def forward(ctx, q, k, v, causal, sm_scale, temp, warp_specialize=False, decode=False):
        ...
        # Decode means a small number of queries (often 1) attending over a longer KV length.
        if decode:
            assert k.shape[2] >= q.shape[2], "Decode expects KV length >= query length"
        else:
            assert k.shape[2] == q.shape[2], "Prefill expects matching sequence lengths"
        extra_kern_args = {}
        ...
        # Allocate outputs and entropy/m log buffers sized to the number of queries.
        M = torch.empty((q.shape[0], q.shape[1], q.shape[2]), device=q.device, dtype=torch.float32)
        E = torch.empty((q.shape[0], q.shape[1], q.shape[2]), device=q.device, dtype=torch.float32)
        ...
        if decode:
            ...
            attn = torch.softmax(attn_scores / temp.unsqueeze(-1), dim=-1)
            ...
            if supports_host_descriptor() and not (is_hopper() and warp_specialize):
                ...
                dummy_block = [1, 1]
                desc_q = TensorDescriptor(q, shape=[y_dim, HEAD_DIM_K], strides=[HEAD_DIM_K, 1],
        ...
        ctx.save_for_backward(q, k, v, o, M, E)
    ...
    @staticmethod
    def backward(ctx, do):
        q, k, v, o, M, E = ctx.saved_tensors

        raise ValueError("if you want to call backward on this, you need to figure out how to "
                         "deal with the entropy term")
